#Importing SQLite, do not need to install like mySQL etc.
import sqlite3

CREATE_TABLE = "CREATE TABLE IF NOT EXISTS beans (id INTEGER PRIMARY KEY, name TEXT, method TEXT, rating INTEGER);"
INSERT_BEAN = "INSERT INTO beans(name, method, rating) VALUES(?, ?, ?);"
GET_ALL_BEANS = "SELECT * FROM beans;"
GET_BEANS_BY_NAME = "SELECT * FROM beans WHERE name = ?;"
GET_BEST_PREPARATION_FOR_BEAN  = """
SELECT * FROM beans 
WHERE name = ? 
ORDER BY rating DESC 
LIMIT 1; """

#Create database and get connection
def connect():
    return sqlite3.connect("data.db")

#When it connects to file, this ensures it will get saved there
def create_tables(connection):
    with connection:
        connection.execute(CREATE_TABLE)

#def add bean 
def add_bean(connection, name, method, rating):
    with connection:
        connection.execute(INSERT_BEAN, (name, method, rating))

#get all beans
def get_all_beans(connection):
    with connection:
        return connection.execute(GET_ALL_BEANS).fetchall()

#get all beans by name 
def get_beans_by_name(connection, name):
    with connection:
        return connection.execute(GET_BEANS_BY_NAME, (name,)).fetchall()

#Get best perparation    
def get_best_perparation(connection, name):
    with connection: 
        return connection.execute(GET_BEST_PREPARATION_FOR_BEAN, (name,)).fetchone()